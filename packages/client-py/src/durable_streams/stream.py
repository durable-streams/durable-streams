...
def _stream_internal(
    ...
    **kwargs: Any,
    ...
    # Parse initial metadata and detect SSE encoding from response header
    headers_dict = parse_httpx_headers(response.headers)
    meta = parse_response_headers(headers_dict)

    # Detect encoding from Stream-SSE-Data-Encoding response header
    encoding = meta.sse_encoding
