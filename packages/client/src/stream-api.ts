...
async function streamInternal<TJson = unknown>(
  ...
  // Detect SSE encoding from response header
  // Server automatically applies base64 encoding for binary streams over SSE
  const isSSEResponse = contentType?.includes(`text/event-stream`)
  const sseEncoding = isSSEResponse
    ? (firstResponse.headers.get(STREAM_SSE_DATA_ENCODING_HEADER) as
        | `base64`
        | null)
    : null
  ...
  return new StreamResponseImpl<TJson>({
    ...
    encoding: sseEncoding ?? undefined,
  })
}
