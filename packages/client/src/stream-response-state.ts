...
// ============================================================================
// New 7-State Electric-Style State Machine
// ============================================================================
...
export abstract class ActiveState extends StreamState {
  ...
  pause(): NewPausedState {
    return new NewPausedState(this)
  }

  toErrorState(error: Error): NewErrorState {
    return new NewErrorState(this, error)
  }
...
export class NewPausedState extends StreamState {
  readonly kind = `paused` as const
  readonly #previousState: ActiveState | NewErrorState

  constructor(previousState: ActiveState | NewErrorState) {
    super()
    if (previousState instanceof NewPausedState) {
  ...
  get previousState(): ActiveState | NewErrorState {
  ...
  resume(): ActiveState | NewErrorState {
    return this.#previousState
  }
  ...
  handleResponseMetadata(input: ResponseMetadataInput): ResponseTransition {
    ...
    if (inner.action === `accepted`) {
      return {
        action: `accepted`,
        state: new NewPausedState(inner.state) as unknown as ActiveState,
      }
    }
...
export class NewErrorState extends StreamState {
  readonly kind = `error` as const
  readonly #previousState: ActiveState | NewPausedState
  readonly error: Error

  constructor(previousState: ActiveState | NewPausedState, error: Error) {
    super()
    if (previousState instanceof NewErrorState) {
      this.#previousState = previousState.#previousState
    } else {
      this.#previousState = previousState
    }
  ...
  get previousState(): ActiveState | NewPausedState {
    return this.#previousState
  }
  ...
  retry(): ActiveState | NewPausedState {
    return this.#previousState
  }
