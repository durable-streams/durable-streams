...
import type { ProviderStatus } from "y-durable-streams"
...
export function useYjsText(name: string): Y.Text {
  const { doc } = useYjsRoom()
  return doc.getText(name)
}

export function useYjsMap<T = unknown>(name: string): Y.Map<T> {
  const { doc } = useYjsRoom()
  return doc.getMap<T>(name)
}

export function useYjsArray<T = unknown>(name: string): Y.Array<T> {
  const { doc } = useYjsRoom()
