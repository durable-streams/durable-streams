...
Servers **SHOULD** return appropriate `Cache-Control` headers based on their caching requirements:

```
Cache-Control: public, max-age=60, stale-while-revalidate=300
```

Servers **MAY** adjust caching directives based on stream sensitivity. See Appendix A for implementation patterns when authentication is required.
...
Authentication and authorization are explicitly out of scope for this protocol specification. The protocol operates over standard HTTP, so implementations **MAY** use any HTTP authentication mechanism (e.g., Basic Authentication [RFC7617], Bearer tokens [RFC6750], Digest Authentication [RFC7616], or custom headers). Implementations **MUST** provide appropriate access controls to prevent unauthorized stream creation, modification, or deletion.

See Appendix A for recommended implementation patterns when combining authentication with CDN caching.
...
This appendix provides non-normative implementation guidance for common deployment scenarios.
...
For truly private data that should not be cached by CDNs, use `Cache-Control: private` to allow browser caching only.
