...
**Fork offset and message boundaries:** The `Stream-Fork-Offset` value **MUST** be an offset previously returned by the server (via `Stream-Next-Offset`), which is always aligned to a message boundary. Since offsets are opaque, clients **MUST NOT** construct or modify offset values. Servers are **NOT REQUIRED** to validate that a fork offset falls on a message boundary. If a client provides a fabricated offset that does not align to a message boundary, the behavior is undefined — reads on the resulting fork **MAY** return corrupted data, incomplete messages, or errors. Servers **MAY** choose to validate boundary alignment and reject invalid offsets with `400 Bad Request`, but this is not required.
