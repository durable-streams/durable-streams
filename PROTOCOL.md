...
**Fork offset and message boundaries:** The `Stream-Fork-Offset` value **SHOULD** be an offset previously returned by the server (via `Stream-Next-Offset`), which is always aligned to a message boundary. Since offsets are opaque, clients **MUST NOT** construct or modify offset values. If a fork offset does not fall on a message boundary, servers **MUST NOT** return an error — instead, the fork inherits only complete messages whose offset is less than the fork offset. This effectively rounds down to the last complete message boundary. No partial messages are ever returned.
