...
**Fork offset and message boundaries:** The `Stream-Fork-Offset` value **SHOULD** be an offset previously returned by the server (via `Stream-Next-Offset`), which is always aligned to a message boundary. Since offsets are opaque, clients **MUST NOT** construct or modify offset values. If a fork offset does not coincide with a message boundary, servers **MUST NOT** return an error. The fork inherits only complete messages whose end-offset is less than or equal to the fork offset — messages that extend past the fork offset are excluded entirely. No partial messages are ever produced. This behavior is identical for all content types including `application/json`.
