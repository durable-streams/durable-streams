...
export function ChatInterface({
  ...
  const connection = useMemo(() => {
    return createDurableConnection(proxyUrl, apiUrl, getStreamKey)
  }, [proxyUrl, apiUrl, getStreamKey])
  ...
  useEffect(() => {
    ...
    const conversationStreams = allKeys
      .filter(
        (k) =>
          k.startsWith(storagePrefix) && k.includes(`${conversationId}-msg-`)
      )
      .map((k) => {
    ...
    if (credentials && credentialsData.readToken) {
      ...
      const streamKey = `${conversationId}-msg-${highestStream.num}`
