...
export default function SnakeGame() {
          ...
          if (newDir.dx !== -s.dir.dx || newDir.dy !== -s.dir.dy) {
            ...
            if (newDir.dx !== s.dir.dx || newDir.dy !== s.dir.dy) {
              ...
              if (inputTimeRef.current) {
                ...
                setTimeout(() => setRoundTrip(Math.round(now - inputTimeRef.current + latency)), latency);
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{
        fontSize: 15, fontWeight: 800, letterSpacing: 4, marginBottom: 2,
        color: PALETTE.snakeHead,
      }}>
        SNAKE
      </div>
      <div style={{ fontSize: 8, color: "#444", marginBottom: 8 }}>
        Obstacles spawn near your head via sync — high latency = less warning time
      </div>

      {/* Controls row */}
      <div style={{
        display: "flex", alignItems: "center", gap: 16, marginBottom: 8,
        background: "#0E1225", padding: "6px 16px", borderRadius: 8,
        border: "1px solid #1A1F38",
      }}>
        <span style={{ fontSize: 9, color: "#555" }}>SYNC LATENCY</span>
        <input type="range" min={0} max={500} step={10} value={latency}
          onChange={e => setLatency(+e.target.value)}
          style={{ width: 120, accentColor: latency < 100 ? PALETTE.accent : latency < 250 ? PALETTE.obsWarn : PALETTE.food }}
        />
        <span style={{
          fontSize: 13, fontWeight: 700, minWidth: 50, textAlign: "right",
          color: latency < 100 ? PALETTE.accent : latency < 250 ? PALETTE.obsWarn : PALETTE.food,
        }}>
          {latency}ms
        </span>
        {roundTrip !== null && (
          <span style={{ fontSize: 8, color: "#666" }}>
            RT <span style={{ color: roundTrip < 150 ? "#0f0" : roundTrip < 350 ? "#ff0" : "#f00" }}>{roundTrip}ms</span>
          </span>
        )}
      </div>
      ...
      {/* Score bar */}
      <div style={{
        display: "flex", gap: 24, marginBottom: 6, fontSize: 10, fontWeight: 600,
      }}>
        <span>SCORE <span style={{ color: PALETTE.accent, fontSize: 13 }}>{game.score}</span></span>
        <span>LENGTH <span style={{ color: PALETTE.accent }}>{game.snake.length}</span></span>
        <span>OBSTACLES <span style={{ color: PALETTE.obsSolid }}>{game.obstacles.length}</span></span>
        <span style={{ color: "#333" }}>HI <span style={{ color: "#555" }}>{highScore}</span></span>
      </div>

      {/* Game board */}
      <div style={{ position: "relative" }}>
        <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}
          style={{ background: PALETTE.grid, borderRadius: 6, border: `2px solid ${PALETTE.gridLine}` }}>
          ...
          {/* Food glow */}
          <circle cx={game.food.x * CELL + CELL / 2} cy={game.food.y * CELL + CELL / 2}
            r={CELL * 1.2} fill={PALETTE.foodGlow} opacity={0.4} />

          {/* Food */}
          <circle cx={game.food.x * CELL + CELL / 2} cy={game.food.y * CELL + CELL / 2}
            r={CELL / 2 - 2} fill={PALETTE.food} />
          <circle cx={game.food.x * CELL + CELL / 2 - 2} cy={game.food.y * CELL + CELL / 2 - 2}
          ...
          {game.obstacles.map((o, i) => {
            ...
            if (isWarning) {
              return (
                <g key={`obs-${i}`}>
                  <rect x={o.x * CELL + 2} y={o.y * CELL + 2} width={CELL - 4} height={CELL - 4}
                    fill="none" stroke={PALETTE.obsWarn} strokeWidth={2} rx={3}
                    strokeDasharray="4 2"
                    style={{ animation: "warnPulse 0.4s ease-in-out infinite" }} />
                  <text x={cx} y={cy + 4} textAnchor="middle" fontSize={12}
                    fill={PALETTE.obsWarn} fontWeight={800}
                    style={{ animation: "warnPulse 0.4s ease-in-out infinite" }}>!</text>
                </g>
              );
            }
            return (
              <g key={`obs-${i}`}>
                <rect x={o.x * CELL + 1} y={o.y * CELL + 1} width={CELL - 2} height={CELL - 2}
                  fill={PALETTE.obsSolid} rx={3} opacity={0.9} />
                <rect x={o.x * CELL + 3} y={o.y * CELL + 3} width={CELL - 6} height={CELL / 2 - 2}
                  fill="rgba(255,255,255,0.12)" rx={2} />
                {/* X mark */}
                <line x1={o.x * CELL + 6} y1={o.y * CELL + 6}
                  x2={o.x * CELL + CELL - 6} y2={o.y * CELL + CELL - 6}
                  stroke="rgba(0,0,0,0.3)" strokeWidth={2} strokeLinecap="round" />
                <line x1={o.x * CELL + CELL - 6} y1={o.y * CELL + 6}
                  x2={o.x * CELL + 6} y2={o.y * CELL + CELL - 6}
                  stroke="rgba(0,0,0,0.3)" strokeWidth={2} strokeLinecap="round" />
              </g>
            );
          })}
          ...
          {/* Snake glow trail */}
          <circle cx={game.snake[0].x * CELL + CELL / 2} cy={game.snake[0].y * CELL + CELL / 2}
            r={CELL * 1.5} fill={PALETTE.snakeGlow} />
                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          ...
                          <circle cx={ex1 + dir.dx * 0.8} cy={ey1 + dir.dy * 0.8} r={1.3} fill="#111" />
                          <circle cx={ex2 + dir.dx * 0.8} cy={ey2 + dir.dy * 0.8} r={1.3} fill="#111" />
                        </>
                      );
                    })()}
                  </>
          ...
          {!gameOver && (
            <circle
              ...
              r={OBS_MAX_DIST * CELL}
              fill="none" stroke="rgba(255,107,53,0.06)" strokeWidth={1}
              strokeDasharray="4 6"
            />
          )}
        </svg>
        ...
        {gameOver && (
          <div style={{
            position: "absolute", inset: 0, background: "rgba(11,14,23,0.92)",
            display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
            gap: 10, borderRadius: 6,
          }}>
            <div style={{ fontSize: 9, color: "#555", letterSpacing: 3 }}>GAME OVER</div>
            <div style={{ fontSize: 28, fontWeight: 800, color: PALETTE.food }}>
              {game.score}
            </div>
            <div style={{ fontSize: 9, color: "#555" }}>
              {game.snake.length} length • {game.obstacles.length} obstacles • {latency}ms latency
            </div>
            {latency > 100 && (
              <div style={{ fontSize: 9, color: PALETTE.obsWarn, maxWidth: 280, textAlign: "center", lineHeight: 1.5 }}>
                At {latency}ms sync latency, obstacles appeared with {Math.round(latency / game.speed * 100) / 100} fewer
                ticks of warning time
              </div>
            )}
            <button onClick={initGame} style={{
              fontFamily: "inherit", fontSize: 10, padding: "8px 28px",
              background: PALETTE.accent, color: "#000", border: "none",
              cursor: "pointer", borderRadius: 4, fontWeight: 700, marginTop: 4,
            }}>
              PLAY AGAIN
            </button>
          </div>
      ...
      {/* Explainer */}
      <div style={{
        fontSize: 8, color: "#333", marginTop: 8, textAlign: "center",
