...
// Demo-first defaults so `pnpm dev` works without extra env setup. The
// streams server is mounted at /streams in this same Worker (src/server.ts),
// so the default points back at the local vite dev server.
const DEFAULT_DURABLE_STREAMS_URL = `http://localhost:3002/streams`
const SHARED_URL =
