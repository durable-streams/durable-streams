...
strict durability guarantee are **unchanged** — only *where/how* the per-stream
...
- Strict acks are gated on a **per-stream** `fdatasync` (`SyncCoalescer::sync_to`):
  ...
  *independent* coalescers → up to thousands of concurrent `spawn_blocking` hops, each a
...
- **Most real workloads have small payloads**, so strict's cost is dominated by these
  per-append *syscalls* + the blocking-pool hop — not by data movement. This is exactly
...
- **Async bridge** (the one new thing vs the WAL committer, which used a watch channel):
  the caller awaits *its* fsync via a per-call `oneshot`. The thread reaps a CQE, looks up
...
- **`oneshot` async bridge, not a watch channel:** unlike the WAL committer (where waiters
  gate on a monotonic watermark), each strict leader awaits *its own* fsync result.
