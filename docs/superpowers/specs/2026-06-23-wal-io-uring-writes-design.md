...
checkpoint, and the durability/no-loss invariants are **unchanged** — only _how_
...
- The WAL append path is **fsync-bound and group-committed**: many concurrent
  ...
  device flush faster, so the per-batch _latency floor_ barely moves. This is a
...
- The `SegmentWriter` trait already exists in `segment.rs` as the documented
  ...
  worse than `pwrite`). Hitting the CPU goal requires a new _committer path_, not
...
- **Bridge to async:** the thread publishes to the **existing** `durable_tx`
  ...
  second runtime is introduced (deliberately _not_ `tokio-uring`, which needs a
...
6. **fsync CQE** → publish `durable_lsn = snapshot` to `durable_tx`, record the
   ...
   verbatim:** records whose write CQE arrived _during_ the fsync are excluded and
...
reaped." Both mean _the bytes are in the page cache for that segment_ — exactly the
