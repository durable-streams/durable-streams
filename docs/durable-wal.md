...
stream's WAL shard (`maybe_sync_on_ack`, `src/handlers.rs`). The ack is held
...
| file           | responsibility                                                                             |
| -------------- | ------------------------------------------------------------------------------------------ |
| `codec.rs`     | B-light record framing (header CRC + payload CRC, always set by the sole WAL writer; flag retained for on-disk format stability); torn-tail detection |
...
(`src/wal/walset.rs`), applied to every record kind, so all of a stream's
...
- **Fixed FNV-1a, not `DefaultHasher`** (`src/wal/walset.rs`). `DefaultHasher`/
...
- **`N` is persisted, not recomputed per boot.** `N` is fixed at data-dir creation
  and written to `<data-dir>/wal/shards` (`SHARDS_FILE`, `src/wal/walset.rs`).
  `WalSet::open` (`src/wal/walset.rs`) reads it; `available_parallelism()` (the
  ...
  data dir a value ≠ the persisted `N` returns `Err` (`src/wal/walset.rs`),
  which `main.rs` maps to **exit 2** (`src/main.rs`).

Default `N` = `std::thread::available_parallelism()` (`src/main.rs`), which on
Linux honors the cgroup `cpu.max` quota — so a pod capped at 4 CPUs gets 4 shards,
not the node's core count.

### Append data path

For each append (`maybe_sync_on_ack`, `src/handlers.rs`):

1. **Per-stream file write** — the wire bytes are written to the stream's own file
   (page cache, **no** hot-path fsync) by `write_wire` (`src/handlers.rs`),
   upstream of the helper. This is the read surface.
2. **Compute the logical `stream_offset`** — `wal_stream_offset`
   (`src/handlers.rs`) returns `file_base + target − wire.len()` (a `u64`), the
...
3. **Register dirty before staging** — `shard.register_dirty(st.id, st)`
   (`src/handlers.rs`) puts the stream in the shard's checkpoint dirty-set
...
stream_offset, wire)` (`src/handlers.rs`) encodes a B-light record and
   `write_at`s it into the active segment.
5. **Ack gate** — `shard.wait_durable(lsn).await` (`src/handlers.rs`) blocks
...
See [LIMITATIONS.md](../packages/server-rust/LIMITATIONS.md) for the buffered-copy
framing context and the removed `--zero-copy` history.
...
u32  payload_crc32c  // crc32c over the payload; always set by the sole WAL writer (flag retained for on-disk format stability)
[len bytes payload]
```
...
`encode_into` is the sole WAL-record writer and always sets `PAYLOAD_CHECKSUMMED` plus a real
payload CRC. The `--zero-copy` splice relay (the only writer that left the flag clear) has
been removed. Every WAL record is therefore checksummed and Bug #1 is **fully closed** — there
is no residual, no opt-out path. The `PAYLOAD_CHECKSUMMED` flag and the flag-clear decode
branch are retained only for on-disk format stability.

As-built note: only `Append` records are actually staged today
(`src/handlers.rs`); `StreamCreate`/`Close`/`Delete` discriminants exist in the
...
(`seg_path`, `src/wal/segment.rs`), each pre-allocated to a fixed
`SEGMENT_BYTES = 128 MiB` (`src/wal/segment.rs`) by `FileSegment::create`
(`src/wal/segment.rs`): `fallocate` on Linux, `set_len` on macOS. Because every
...
**Roll** (`reserve_and_stage`, `src/wal/shard.rs`): if a record would overflow
the active segment, the appender (under the short reserve lock):

1. **Seals** the current segment — `seal_to(write_pos)` (`src/wal/segment.rs`)
   ...
   seeing a zero gap.
2. Pushes `(end_lsn, sealed_segment)` onto `sealed_pending` (`src/wal/shard.rs`).
...
the active one before advancing `durable_lsn` (`run_committer`, `src/wal/shard.rs`), and prunes
a sealed entry only **after** the fsync that covered it
(`run_committer`, `src/wal/shard.rs`). This is the off-lock-write-after-seal fix; without it the
...
Each shard runs one `run_committer` task (`src/wal/shard.rs`): park on a
...
`src/wal/shard.rs`). Appenders run two phases: a short locked **reserve**
...
off-lock, LSN `k+1` can finish before `k`. `mark_written` (`src/wal/shard.rs`)
...
the **pre-fsync** snapshot exactly (`run_committer`, `src/wal/shard.rs`), never re-snapshotting,
so it can never advance past a record the fsync did not cover. On any fsync error
it does **not** advance and does **not** ack (`run_committer`, `src/wal/shard.rs`). A failed
`write_at` returns `Err` (the ack fails) rather than panicking
(`reserve_and_stage`, `src/wal/shard.rs`) — the `reserve_and_stage → Result` fix-wave change; the
...
`Shard::checkpoint` (`src/wal/shard.rs`), driven by a 3 s ticker
(`CHECKPOINT_INTERVAL`, `src/main.rs`; `spawn_checkpoint_ticker`,
`src/main.rs`), is the per-stream fsync _relocated off the ack path_, batched
and N-way parallel. Its **hard ordering**:

1. Snapshot `checkpoint_lsn = durable_lsn` (`Shard::checkpoint`, `src/wal/shard.rs`).
2. **Drain the dirty set** — `dirty: HashMap<stream_id, Arc<StreamState>>`
   (`src/wal/shard.rs`), deduped by id. `std::mem::take` it, read each stream's
   ...
   (`Shard::checkpoint`, `src/wal/shard.rs`). Draining first means a concurrent re-touch lands in the
   _next_ checkpoint, never silently dropped.
3. **Persist per-stream durable-tails** — `persist_durable_tails`
   (`src/wal/shard.rs`) merges each touched stream's durable tail into the
   cumulative `<shard_dir>/tails` map (`TAILS_FILE`, `src/wal/shard.rs`),
   `tmp`+rename, **fsync'd before recycle**.
4. **Persist `checkpoint_lsn`** to `<shard_dir>/checkpoint` (`tmp`+rename,
   `src/wal/shard.rs`).
5. **Recycle** — `recycle_below` (`src/wal/shard.rs`) unlinks every segment
...
fatal (`src/main.rs`); a failed unlink leaves over-retention, never loss
(`recycle_below`, `src/wal/shard.rs`), and the next checkpoint retries.
...
WAL recovery (`recovery::recover`, `src/wal/recovery.rs`) runs at boot **after**
the non-sharded sidecar pass that owns stream identity (`Store::recover` inside
`new_with_tier`, `src/store.rs`) and **before** any append. It builds a
`stream_id → Arc<StreamState>` index from the sidecar-recovered streams, then
spawns one OS thread per shard (`src/wal/recovery.rs`) — shards own disjoint
...
Per shard (`recover_shard`, `src/wal/recovery.rs`):

- **Replay from the oldest _retained_ record, not from `checkpoint_lsn`**
  (`replay_from_checkpoint(0, ...)`, `src/wal/recovery.rs`). This is the **C1
  ...
  read (`src/wal/recovery.rs`) but used only as a write-skip optimization — the
  bound is `0`.
- **Seed the frontier from the persisted durable-tails** (`read_durable_tails`,
  `src/wal/recovery.rs`), so a stream whose WAL records were all recycled is
  still reconciled (the roll-era hole).
- For each `Append`, resolve `stream_id → StreamState`; **frontier skip** if
  `stream_offset < file_base` (`recover_shard`, `src/wal/recovery.rs`) — those bytes are already
  ...
  (`recover_shard`, `src/wal/recovery.rs`) and raise the stream's frontier to
...
max(persisted_tail, replayed)` (`reconcile_tail`, `src/wal/recovery.rs`):
  ...
  extended it. Then **`fdatasync` the repair** (`reconcile_tail`, `src/wal/recovery.rs`) so a
  crash after recovery but before the next checkpoint can't lose it, and publish
  `Shared.tail` + the appender `written`.

After recovery, `reset_after_recovery` (`src/wal/walset.rs`,
`src/wal/shard.rs`) wipes each shard's WAL to a fresh zero-filled segment at
...
_second_ crash would mis-replay. The boot order (`src/main.rs`) is exactly:
...
Both the committer's `fdatasync` (`FileSegment::fdatasync`, `src/wal/segment.rs`)
and the seal's `fsync` (`seal_to`, `src/wal/segment.rs`) use **macOS
...
commit by `ShardStats::record_batch(watermark − durable)` (`src/wal/shard.rs`,
`src/wal/telemetry.rs`) — a handful of relaxed atomic adds plus one bucketed
`fetch_add` (buckets `[1,2,4,8,16,32,64,128]`, `src/wal/telemetry.rs`), no lock/
alloc/syscall on the commit path. A 1 Hz emitter (`spawn_emitter`,
`src/wal/telemetry.rs`) prints `WAL_STATS shard=<i> batch_p50/p99/max/avg
...
(`src/wal/telemetry.rs`), so a default build pays only the per-commit atomics.
...
`SegmentWriter` (`src/wal/segment.rs`) abstracts a shard's WAL I/O down to two
...
| 3   | Batched group commit, N parallel committers                | **HOLDS**             | One `run_committer` per shard (`spawn_committers`, `src/wal/walset.rs`); each fat-batches a fsync; the cardinality sweep confirms cardinality-insensitive batching.                                                                                                                                |
| 4   | WAL-only ack; `stream_offset` computed under appender lock | **HOLDS** (after fix) | The whole-branch review caught a regression — `stream_offset` was computed (taking `st.shared.read()`) unconditionally; the fix-wave moved it into `wal_stream_offset` (`src/handlers.rs`). Now verified.                                                                                          |
...
| 7   | Persisted-N, FNV-1a routing, mismatch → exit 2             | **HOLDS**             | `src/wal/walset.rs` (FNV-1a, persisted-N, mismatch guard); `src/main.rs` (exit 2 mapping). N stable across reopen with a different `available_parallelism`.                                                                                                                                                                                                                                                                                                                        |

### Where the build extended the spec (as-built additions)

- **Segment roll + `sealed_pending`** (`reserve_and_stage`, `sealed_pending`, `run_committer` in `src/wal/shard.rs`) — not in the
...
- **Per-stream durable-tails at checkpoint** (`persist_durable_tails`, `TAILS_FILE` in `src/wal/shard.rs`) — not in the spec's checkpoint; required once recycle fires so a
...
- **`F_FULLFSYNC` on macOS** (`src/wal/segment.rs`) — the spec named Linux
  ...
  "no-loss" contract.
- **`reserve_and_stage → io::Result`** (`src/wal/shard.rs`) — a transient WAL
...
- Deferred speed-ups (io_uring `SegmentWriter`, CPU pinning,
  dynamic rebalancing, replication, checkpoint-cadence tuning) are tracked in
  `docs/superpowers/durability-performance-followups.md`. Payload CRC is implemented
  and always set by the sole WAL writer (`encode_into`); Bug #1 is fully closed with
  no residual. (The `--zero-copy` splice relay — the only writer that ever left the
  flag clear — has been removed.)
